                ...
                <div class="border-2 p-2 mx-2 border-gray-300 rounded-md shadow-sm">
                    ...
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <small class="text-xs">{{element.size}}</small>
                        </div>
                        <i class="mdi mdi-delete-outline text-xl cursor-pointer" aria-hidden="true" @click="removeFile(element.custom_id)"></i>
                    </div>
                </div>
            </div>
        </template>
        ...
        computed:{
            canShow() {
                if (this.files.length === 0) {
                    return true;
                } else if (this.files.length > 0 && this.isMultiple ) {
                    return true;
                }
                return false;
            }
            ...
            addFile(file) {
                 if (this.isAcceptableFileType(file.type)) {
                    if (file.type.match('image.*')) {
                        ...
                        // TODO: Need to load correct static preview image according to the file type.
                        file.preview_url = '#';
                    }
